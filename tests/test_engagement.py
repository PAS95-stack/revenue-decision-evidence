...
class ConfigValidationTests(EngagementTestCase):
    ...
    def test_e06_invalid_configs_are_refused_with_the_reason(self):
        ...
        cases = [
            ...
            (lambda c: ads(c)[0].update(date_format="DD/MM"), "date_format is written from"),
            (lambda c: ads(c)[0].update(date_format="QQ/MM/YYYY"), "date_format is written from"),
            (lambda c: ads(c)[0].update(date_format=["DD-MM-YYYY", "MM-DD-YYYY"]), "day-first and a month-first"),
            ...
            (lambda c: ads(c)[1]["amounts"].update(currency_label="AED 1"), "no digits"),
            (lambda c: ads(c)[1]["amounts"].update(currency_label="a much too long label"), "up to 8 characters"),
            (lambda c: ads(c)[0]["amounts"].update(decimal="dot"), 'decimal must be "point"'),
            (lambda c: ads(c)[1]["amounts"].update(decimal="comma"), "thousands_separator must be '.'"),
            (lambda c: ads(c)[0].update(header_row=0), "header_row must be a whole number"),
            (lambda c: ads(c)[0].update(header_row="3"), "header_row must be a whole number"),
